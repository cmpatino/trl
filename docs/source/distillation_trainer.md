...
| FSDP2 (student) | Validation in progress, not yet supported | GPU (2× A10G, bf16) on tiny fixtures: forced-eviction gradient parity and the one-head device memory bound measured (head-cache counter evidence on the device). The student output head remains a plain (unsharded) parameter on Accelerate 1.14.0 (the tail parameter group is never resharded), so the `DTensor.full_tensor()` head-materialization path is not exercised. Managed-vs-legacy update parity is pending a clean two-rank run with per-rank weight and target checks: the earlier comparison (a reported 6.1e-6, i.e. 0.012 bf16-ULP, update difference attributed to reduction order) has been withdrawn, because the legacy reference teacher in those runs was zeroed on rank 1 by Accelerate's process-global RAM-efficient-loading flags |
