...
| FSDP2 (student) | Validation in progress, not yet supported | GPU (two processes on two of four L4 GPUs, bf16) on tiny fixtures: a clean two-rank run, with the legacy teacher's RAM-efficient loading isolated and per-rank weight checksums verified equal on both ranks, gives managed-vs-legacy update parity **bitwise** — parameter difference 0, losses bitwise, legacy repeat bitwise, generation identical, teacher targets identical. Forced-eviction gradient parity and the one-head device memory bound also measured (head-cache counter evidence on the device). The student output head remains a plain (unsharded) parameter on Accelerate 1.14.0 (the tail parameter group is never resharded), so the `DTensor.full_tensor()` head-materialization path is not exercised by this evidence |
| Ten-teacher scale | Validation in progress, not yet supported | GPU (1 and 2× A10G), representative-size teachers: student `Qwen/Qwen2.5-0.5B`, ten registered IDs over seven distinct 0.5B–3B checkpoints plus three repeated repositories. Run limits: four optimizer steps, batch size 2 per rank, gradient accumulation 2, completions capped at 64 tokens — so a scoring window holds at most four rows per rank and not all ten IDs are active in any one window; the two-rank timings below are not a strong-scaling comparison. On one A10G: peak device head bytes 272/272/467/622 MB for 1/2/5/10 registered IDs (= the largest registered head in bf16; head-cache counter evidence, corroborated by CUDA allocator totals that independently show the run fits — device max allocated 5.9–8.3 GB); host PSS max (0.5 s sampler over the training call, which can miss short spikes) 7.1/8.3/10.2/13.6 GB; CPU body reloads 0/6/10/14 over the four scoring windows; 3.9/5.2/9.4/12.0 s per optimizer step. On two A10Gs: 4.5/6.8/11.8/17.2 s per step, one-head counter holds on every rank. Timed `_load_body` calls consumed 27.3 of 48.0 s (57%) of the training call on one rank at ten IDs, and 37.0 of 68.6 s (54%) on rank 0 of the two-rank run at ten IDs: loading dominates the training call, and host-to-device transfers add to it. Larger models, ten *distinct* representative teachers, larger active windows, multi-node, and vLLM remain unmeasured |
...
ten-teacher-scale row is the exception, measured directly at representative sizes. Larger models, ten *distinct*
representative teachers (the measured run reuses three repositories under distinct IDs to reach ten), larger active
scoring windows, multi-node runs, and vLLM generation interplay remain future validation, not implicit claims.
