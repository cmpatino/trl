...
| Ten-teacher scale | Validation in progress, not yet supported | GPU (1 and 2 ranks) on tiny fixtures: peak device head bytes (head-cache counter evidence on the device) are constant across 1/2/5/10 registered teachers; teacher body loads grow with the number of *active* teachers per window; per-rank host PSS measured as a post-training snapshot (peak PSS during loading is not measured). Representative (multi-GB) teacher sizes have not been measured |
